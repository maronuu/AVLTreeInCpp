...
public:
    ...
    Node(int height_, K key_, V val_)
        : height(height_), key(key_), val(val_)
    {
        left = nullptr;
        right = nullptr;
    }
...
public:
    ...
    bool is_large(const K key, Node<K, V> *t) {
        ...
        return (
            (key < t->key) &&
            is_large(key, t->left) &&
            is_large(key, t->right)
        );
    }
    bool is_valid_bst(Node<K, V> *t) {
        if (t == nullptr) return true;
        return (
            is_small(t->key, t->left) &&
            is_large(t->key, t->right) &&
            is_valid_bst(t->left) &&
...
int main() {
    ...
    for (int key: gt.keys()) {
        ...
        if (tree.is_member(key)) num_delete_err++;
    }
